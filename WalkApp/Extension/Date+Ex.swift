    ...
    */
    ...
    func toStringWithRelativeTime(strings:[RelativeTimeStringType:String]? = nil) -> String {
        ...
        if sec < 60 {
            if sec < 10 {
                if isPast {
                    return strings?[.nowPast] ?? NSLocalizedString("지금", comment: "Date format")
                } else {
                    return strings?[.nowFuture] ?? NSLocalizedString("지금", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("%.f초 전", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("%.f초 전", comment: "Date format")
                }
        ...
        if min < 60 {
            if min == 1 {
                if isPast {
                    return strings?[.oneMinutePast] ?? NSLocalizedString("1분 전", comment: "Date format")
                } else {
                    return strings?[.oneMinuteFuture] ?? NSLocalizedString("1분 전", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
                    string = strings?[.minutesPast] ?? NSLocalizedString("%.f분 전", comment: "Date format")
                } else {
                    string = strings?[.minutesFuture] ?? NSLocalizedString("%.f분 전", comment: "Date format")
                }
        ...
        if hr < 24 {
            if hr == 1 {
                if isPast {
                    return strings?[.oneHourPast] ?? NSLocalizedString("1시간", comment: "Date format")
                } else {
                    return strings?[.oneHourFuture] ?? NSLocalizedString("1시간", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
                    string = strings?[.hoursPast] ?? NSLocalizedString("%.f시간 전", comment: "Date format")
                } else {
                    string = strings?[.hoursFuture] ?? NSLocalizedString("%.f시간 전", comment: "Date format")
                }
        ...
        if d < 7 {
            if d == 1 {
                if isPast {
                    return strings?[.oneDayPast] ?? NSLocalizedString("어제", comment: "Date format")
                } else {
                    return strings?[.oneDayFuture] ?? NSLocalizedString("어제", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
                    string = strings?[.daysPast] ?? NSLocalizedString("%.f일 전", comment: "Date format")
                } else {
                    string = strings?[.daysFuture] ?? NSLocalizedString("%.f일 전", comment: "Date format")
                }
        ...
        if d < 28 {
            if isPast {
                if compare(.isLastWeek) {
                    return strings?[.oneWeekPast] ?? NSLocalizedString("지난 주", comment: "Date format")
                } else {
                    let string = strings?[.weeksPast] ?? NSLocalizedString("%.f주 전", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .week))))
                }
            } else {
                if compare(.isNextWeek) {
                    return strings?[.oneWeekFuture] ?? NSLocalizedString("다음 주", comment: "Date format")
                } else {
                    let string = strings?[.weeksFuture] ?? NSLocalizedString("다음 주", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .week))))
                }
            }
        }
        if compare(.isThisYear) {
            if isPast {
                if compare(.isLastMonth) {
                    return strings?[.oneMonthPast] ?? NSLocalizedString("지난 달", comment: "Date format")
                } else {
                ...
                if compare(.isNextMonth) {
                    return strings?[.oneMonthFuture] ?? NSLocalizedString("다음 달", comment: "Date format")
                } else {
                    let string = strings?[.monthsFuture] ?? NSLocalizedString("다음 달", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .month))))
                }
            }
        }
        if isPast {
            if compare(.isLastYear) {
                return strings?[.oneYearPast] ?? NSLocalizedString("작년", comment: "Date format")
            } else {
            ...
            if compare(.isNextYear) {
                return strings?[.oneYearFuture] ?? NSLocalizedString("내년", comment: "Date format")
            } else {
                let string = strings?[.yearsFuture] ?? NSLocalizedString("내년", comment: "Date format")
                return String(format: string, Double(abs(since(Date(), in: .year))))
            }
        }
    }
    ...
    func isIn(start: Date, end: Date) -> Bool {
        if start.compare(self) == .orderedDescending { return false }
        if end.compare(self) == .orderedAscending { return false }
        return true
    }
    
    func isIn(start: String, end: String, format: String) throws -> Bool {
        let formatter = DateFormatter()
        formatter.dateFormat = format
        formatter.timeZone = TimeZone(abbreviation: "UTC")
        guard let _start = formatter.date(from: start), let _end = formatter.date(from: end) else { throw ConvertError.invalidTarget }
        return isIn(start: _start, end: _end)
    }
}
